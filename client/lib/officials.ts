...
const sourceToOfficeType: Record<
  SourceType,
  "tx_senate" | "tx_house" | "us_house" | "statewide"
> = {
  TX_HOUSE: "tx_house",
  TX_SENATE: "tx_senate",
  US_HOUSE: "us_house",
...
export function districtTypeToSourceType(
...
export function getOfficeTypeLabel(
  officeType: string,
...
export function normalizeOfficial(raw: Record<string, unknown>): Official {
  ...
  const districtNumber = parseInt(
    String(raw.district || raw.districtNumber || 0),
    10,
  );
  const isVacant =
    raw.isVacant === true ||
  ...
  return {
    ...
    roleTitle: raw.roleTitle
      ? String(raw.roleTitle)
      : raw.role_title
        ? String(raw.role_title)
        : null,
    ...
    photoUrl: raw.photoUrl
      ...
      : raw.photo_url
        ...
        : null,
    capitolPhone: raw.capitolPhone
      ? String(raw.capitolPhone)
      : raw.capitol_phone
        ? String(raw.capitol_phone)
        : null,
    capitolAddress: raw.capitolAddress
      ? String(raw.capitolAddress)
      : raw.capitol_address
        ? String(raw.capitol_address)
        : null,
    ...
    districtAddresses: Array.isArray(raw.districtAddresses)
      ...
      : Array.isArray(raw.district_addresses)
        ? raw.district_addresses
        : [],
    districtPhones: Array.isArray(raw.districtPhones)
      ? raw.districtPhones
      : Array.isArray(raw.district_phones)
        ? raw.district_phones
        : [],
    isVacant,
    officeType,
