...
function parseRoomFromCapitolAddress(
...
export function apiOfficialsToNormalized(
  apiOfficials: MergedOfficial[],
): Official[] {
...
export function mockOfficialToNormalized(mockOfficial: MockOfficial): Official {
  return normalizeOfficial({
    id: mockOfficial.id,
    source:
      mockOfficial.officeType === "tx_house"
        ? "TX_HOUSE"
        : mockOfficial.officeType === "tx_senate"
          ? "TX_SENATE"
          : "US_HOUSE",
...
export function mockOfficialsToNormalized(
  mockOfficials: MockOfficial[],
): Official[] {
...
export function getApiSourceFromDistrictType(
...
export function apiOfficialToLegacy(apiOfficial: MergedOfficial): MockOfficial {
  ...
  const capitolRoom =
    apiOfficial.capitolRoom ||
    parseRoomFromCapitolAddress(apiOfficial.capitolAddress);
  ...
  if (
    apiOfficial.districtAddresses?.length ||
    apiOfficial.districtPhones?.length
  ) {
    offices.push({
