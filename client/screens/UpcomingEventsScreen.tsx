...
function getRelativeBadge(dateStr: string): {
  label: string;
  color: string;
  type: "past" | "today" | "soon" | "future";
} {
  const d = new Date(dateStr);
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  ...
  if (diffDays === 0)
    return { label: "Today", color: "warning", type: "today" };
  if (diffDays <= 7)
    return { label: "This Week", color: "success", type: "soon" };
    ...
    return (
      ...
      >
        <View style={styles.cardHeader}>
          <View
            style={[styles.badgePill, { backgroundColor: badgeColor + "18" }]}
          >
            <ThemedText
              type="small"
              style={{ color: badgeColor, fontWeight: "700" }}
            >
              {badge.label}
            </ThemedText>
          </View>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              ...
              >
                No upcoming events.{"\n"}Set event dates on your prayers to see
                them here.
              </ThemedText>
            </View>
          }
