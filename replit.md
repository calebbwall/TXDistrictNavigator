...
- **Targeted Refresh Utilities**: `server/jobs/targetedRefresh.ts` exports `refreshChamberUpcomingHearings(chamber)` (discovers hearings from TLO meetings list), `refreshCommitteeHearings(committeeId, windowDays)` (scoped to one committee), `refreshHearingDetail(eventId)` (parses agenda items from notice pages), and `refreshBillHistory(billNumber)` (bill action history). Uses cheerio for HTML parsing, SHA256 fingerprints for change detection, sequential fetches with retry (up to 3 attempts, exponential backoff).
...
- **Admin Trigger Endpoints**: `POST /api/admin/bootstrap-legislative` seeds committees → RSS feeds → events in sequence (idempotent). `POST /api/admin/run-hourly` manually triggers the RSS poll job. `POST /api/admin/run-daily` manually triggers the daily legislative refresh. All guarded by `ADMIN_CRON_SECRET` env var — provide via `x-admin-secret` header or `Authorization: Bearer <value>`. If `ADMIN_CRON_SECRET` is unset, endpoints are open (dev mode).
