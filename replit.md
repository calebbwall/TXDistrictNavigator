...
- **Texas Tribune Headshots**: TX House and TX Senate officials have headshot photos auto-populated from the Texas Tribune directory (`/static/images/headshots/`). Backfill available via `POST /admin/backfill/headshots`. New officials get headshots automatically during refresh. Existing photos are never overwritten. Profile photos are tappable for full-screen viewing via a lightbox modal. Headshot markers on the map use **three placement modes**: (1) **Tap mode** uses closest-point-inside-polygon placement — markers appear at the tap point if inside the district, otherwise snap to the nearest boundary point and nudge inward toward the centroid. (2) **Draw mode** uses precomputed interior anchor points — each district gets 1-6 anchors (based on area / 0.25 deg²) selected via grid sampling and greedy farthest-point algorithm, preventing border clumping when multiple districts are selected via draw-to-search. (3) **Collision avoidance** uses spiral nudging (6 angles × 6 radii) to prevent markers from overlapping, validating candidates stay inside the feature boundary. Both client MAP_HTML (native) and server getMapHtml() (web) implement all three modes with cached anchors, boundary rings, and feature lookups for performance.
