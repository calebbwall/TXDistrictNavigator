 ...
 */
export function generateOtherTxSourceMemberId(
  roleTitle: string,
  fullName: string,
  _category?: string,
): string {
...
async function scrapeExecutiveOfficials(): Promise<OtherTexasOfficialData[]> {
  ...
  console.log(
    `[OtherTxScrape] Executive: using ${officials.length} officials from curated data`,
  );
  return officials;
}
...
async function scrapeSupremeCourt(): Promise<OtherTexasOfficialData[]> {
  ...
  try {
    ...
    const justicePattern =
      /justices\/(chief-justice|justice)-([^\/]+)\/"[^>]*>([^<]+)</gi;
    ...
    if (chiefMatch) {
      ...
      if (name.length > 2 && !seenNames.has(name)) {
        ...
        officials.push({
          ...
          capitolAddress:
            "Supreme Court of Texas, P.O. Box 12248, Austin, TX 78711",
    ...
    if (officials.length < 9) {
      console.log(
        `[OtherTxScrape] Supreme Court: only found ${officials.length}, using static fallback`,
      );
      return getStaticSupremeCourt();
    }

    console.log(
      `[OtherTxScrape] Supreme Court: found ${officials.length} justices`,
    );
    return officials;
  } catch (error) {
...
async function scrapeCriminalAppeals(): Promise<OtherTexasOfficialData[]> {
  ...
  try {
    ...
    if (presidingMatch) {
      ...
      if (!seenNames.has(name)) {
        ...
        officials.push({
          ...
          capitolAddress:
            "Court of Criminal Appeals, P.O. Box 12308, Austin, TX 78711",
    ...
    if (officials.length < 9) {
      console.log(
        `[OtherTxScrape] Criminal Appeals: only found ${officials.length}, using static fallback`,
      );
      return getStaticCriminalAppeals();
    }

    console.log(
      `[OtherTxScrape] Criminal Appeals: found ${officials.length} judges`,
    );
    return officials;
  } catch (error) {
  ...
  return [
    {
      ...
      capitolAddress:
        "Office of the Governor, P.O. Box 12428, Austin, TX 78711",
...
export async function fetchAllOtherTexasOfficials(): Promise<OtherTxScrapedData> {
  ...
  const sources = {
    ...
    supremeCourt: {
      url: "https://www.txcourts.gov/supreme/about-the-court/",
      success: false as boolean,
      error: undefined as string | undefined,
    },
    criminalAppeals: {
  ...
  const sortedForFingerprint = [...allOfficials].sort((a, b) =>
    `${a.category}-${a.roleTitle}-${a.fullName}`.localeCompare(
      `${b.category}-${b.roleTitle}-${b.fullName}`,
    ),
  );
  const fingerprintData = sortedForFingerprint
    .map((o) => `${o.category}|${o.roleTitle}|${o.fullName}`)
    .join("\n");
  ...
  console.log(
    `[OtherTxScrape] Complete: ${allOfficials.length} officials fetched (${duration}ms)`,
  );
  console.log(
    `[OtherTxScrape] Breakdown: ${executive.length} executive, ${supremeCourt.length} Supreme Court, ${criminalAppeals.length} Criminal Appeals`,
  );
