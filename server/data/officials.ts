...
const usCongressNamesMap = loadNamesFromGeoJSON(
  "us_congress_simplified.geojson",
);
...
function getParty(name: string, democratsSet: Set<string>): "R" | "D" {
  ...
  for (const dem of democratsSet) {
    ...
    if (nameParts.length >= 2 && demParts.length >= 2) {
      if (
        nameParts[nameParts.length - 1] === demParts[demParts.length - 1] &&
...
export const txHouseOfficials = generateOfficials(
  txHouseNamesMap,
  150,
  "tx_house",
  txHouseDemocrats,
);
export const txSenateOfficials = generateOfficials(
  txSenateNamesMap,
  31,
  "tx_senate",
  txSenateDemocrats,
);
export const usCongressOfficials = generateOfficials(
  usCongressNamesMap,
  38,
  "us_congress",
  usCongressDemocrats,
...
export function getOfficialsByDistrict(
  chamber: DistrictType,
  districtNumber: number,
): Official | undefined {
  return allOfficials.find(
    (o) => o.chamber === chamber && o.districtNumber === districtNumber,
  );
}
