  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (
        ...
        console.log(
          `[BulkFill] DB retry ${attempt}/${maxRetries} (${label}): ${msg}`,
        );
...
export async function bulkFillHometowns(): Promise<BulkFillResult> {
  ...
  const allPrivate = await dbQuery(
    () =>
      db
        .select({
    ...
    "fetch existing private records",
  );

  const coveredOfficialIds = new Set(
    allPrivate.map((p) => p.officialPublicId).filter(Boolean),
  );
  const coveredPersonIds = new Set(
    allPrivate.map((p) => p.personId).filter(Boolean),
  );

  const allOfficials = await dbQuery(
    () =>
      db
        .select({
  ...
  const officials = uncheckedOfficials.sort(
  ...
  if (officials.length === 0) {
    console.log(
      `[BulkFill] All ${allOfficials.length} officials already have private records. Nothing to do.`,
    );
    return result;
  }

  console.log(
    `[BulkFill] Found ${officials.length} unchecked officials (of ${allOfficials.length} total)`,
  );
  ...
  async function processOne(
    ...
    try {
      ...
      if (!lookup.success || !lookup.hometown) {
        await dbQuery(
          () =>
            db.insert(officialPrivate).values({
        ...
        console.log(
          `[BulkFill] Not found, marked checked: ${official.fullName}`,
        );
        result.notFound++;
        result.details.push({
          ...
          status: "not_found",
          reason:
            "Not found in Texas Tribune directory (marked so it won't be re-checked)",
        });
        return;
      }
      ...
      console.log(
        `[BulkFill] Created ${official.fullName}: ${lookup.hometown}`,
      );
  ...
  for (let i = 0; i < officials.length; i += CONCURRENCY) {
    ...
    if (
      processed % PROGRESS_LOG_EVERY === 0 ||
      processed === officials.length
    ) {
      console.log(
        `[BulkFill] Progress: ${processed}/${officials.length} (filled=${result.filled}, notFound=${result.notFound})`,
      );
    }
  ...
  console.log(
    `[BulkFill] Complete! Filled: ${result.filled}, Skipped: ${result.skipped}, Not Found: ${result.notFound}, Errors: ${result.errors}`,
  );
