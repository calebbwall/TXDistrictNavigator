...
async function main(): Promise<void> {
  ...
  if (officialsRow.n === 0) {
    ...
    console.log(
      `[PreStartSeed] Step 1/4: Officials already populated (${officialsRow.n}) — skipping`,
    );
  }
  ...
  if (committeesRow.n === 0) {
    ...
    console.log(
      `[PreStartSeed] Step 2/4: Committees already populated (${committeesRow.n}) — skipping`,
    );
  }

  // ── Step 4: Hometowns ──
  const [updatedOfficialsRow] = await db
    .select({ n: sql<number>`count(*)::int` })
    .from(officialPublic);
  if (hometownsRow.n < updatedOfficialsRow.n * 0.5) {
    // If fewer than half of officials have hometown records, run backfill
    console.log(
      `[PreStartSeed] Step 3/4: Hometowns incomplete (${hometownsRow.n} of ${updatedOfficialsRow.n}) — running backfill...`,
    );
    const { bulkFillHometowns } = await import("./bulkFillHometowns");
    const result = await bulkFillHometowns();
    console.log(
      `[PreStartSeed] Hometown backfill: filled=${result.filled}, notFound=${result.notFound}, errors=${result.errors}`,
    );
  } else {
    console.log(
      `[PreStartSeed] Step 3/4: Hometowns sufficiently populated (${hometownsRow.n}) — skipping`,
    );
  }
  ...
  if (eventsRow.n === 0) {
    ...
    console.log(
      `[PreStartSeed] Daily refresh: +${eventResult.newEvents} new events, ${eventResult.alertsCreated} alerts`,
    );
  } else {
    console.log(
      `[PreStartSeed] Step 4/4: Legislative events already populated (${eventsRow.n}) — skipping`,
    );
  }
  ...
  console.log("========================================");
  console.log(
    `[PreStartSeed] END pre-start seed complete in ${Math.round(duration / 1000)}s`,
  );
  console.log("========================================");
