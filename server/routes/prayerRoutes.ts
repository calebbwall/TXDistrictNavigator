...
async function getAutoArchiveEnabled(): Promise<boolean> {
  const row = await db
    .select()
    .from(appSettings)
    .where(eq(appSettings.key, "autoArchiveEnabled"))
    .limit(1);
...
async function getAutoArchiveDays(): Promise<number> {
  const row = await db
    .select()
    .from(appSettings)
    .where(eq(appSettings.key, "autoArchiveDays"))
    .limit(1);
...
export function registerPrayerRoutes(app: Express) {
  ...
  app.get("/api/prayers", requireUser, async (req: Request, res: Response) => {
    try {
      ...
      if (status && status !== "ALL") {
        conditions.push(
          eq(prayers.status, status as "OPEN" | "ANSWERED" | "ARCHIVED"),
        );
      }
      ...
      if (q && typeof q === "string" && q.trim()) {
        const search = `%${q.trim()}%`;
        conditions.push(
          or(ilike(prayers.title, search), ilike(prayers.body, search)),
        );
      }
      if (officialId && typeof officialId === "string") {
        conditions.push(
          sql`${prayers.officialIds}::jsonb @> ${JSON.stringify([officialId])}::jsonb`,
        );
      }
      ...
      const orderBy =
        sort === "needsAttention"
          ? [
              asc(prayers.lastPrayedAt),
              desc(prayers.priority),
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        res.setHeader(
          "Content-Disposition",
          "attachment; filename=prayers-export.csv",
        );
        res.send(csv);
      } catch (err: any) {
        res.status(500).json({ error: err.message });
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const result = await db
          ...
          .from(prayers)
          .where(
            and(eq(prayers.userId, userId), eq(prayers.status, "ANSWERED")),
          )
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (groupBy === "officials") {
          ...
          for (const p of allPrayers) {
            ...
            if (ids.length === 0) {
              officialCounts.set(
                "__none__",
                (officialCounts.get("__none__") || 0) + 1,
              );
            } else {
          ...
          const groups = Array.from(officialCounts.entries()).map(
            ([id, count]) => ({
              id,
        ...
        if (groupBy === "categories") {
          ...
          const groups = Array.from(categoryCounts.entries()).map(
            ([id, count]) => ({
              id,
              name:
        ...
        res
          .status(400)
          .json({ error: "groupBy must be 'officials' or 'categories'" });
      } catch (err: any) {
        res.status(500).json({ error: err.message });
      }
    },
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (updates.eventDate !== undefined) {
          updates.eventDate = updates.eventDate
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!action || !Array.isArray(prayerIds) || prayerIds.length === 0) {
          return res
            .status(400)
            .json({ error: "action and prayerIds[] required" });
        }
        ...
        const result = await db
          ...
          .set(updates)
          .where(
            and(eq(prayers.userId, userId), inArray(prayers.id, prayerIds)),
          )
          .returning();
    ...
    async (req: Request, res: Response) => {
      try {
            ...
            const prayerList =
              ids.length > 0
                ? await db
                    ...
                    .from(prayers)
                    .where(
                      and(eq(prayers.userId, userId), inArray(prayers.id, ids)),
                    )
                : [];
            const ordered = ids
              .map((id) => prayerList.find((p) => p.id === id))
              .filter(Boolean);
            return res.json({
              dateKey: todayKey,
              prayers: ordered,
        ...
        for (const row of recentPickRows) {
          if (row.dateKey === yesterdayKey)
            yesterdayIds.push(...(row.prayerIds as string[]));
          if (row.dateKey === twoDaysAgoKey)
            twoDaysAgoIds.push(...(row.prayerIds as string[]));
        }
        const recentIds = new Set([...yesterdayIds, ...twoDaysAgoIds]);

        const openPrayers = await db
          .select()
          .from(prayers)
          .where(and(eq(prayers.userId, userId), eq(prayers.status, "OPEN")))
        ...
        if (picks.length < 3) {
          ...
          const strictEligible = openPrayers.filter(
            (p) => !pickedIds.has(p.id) && !recentIds.has(p.id),
          );
          ...
          if (pool.length > 0) {
            const weighted = pool.map((p) => {
              ...
              if (p.lastShownAt === null) weight += 5;
                ...
                const daysSince =
                  (Date.now() - p.lastShownAt.getTime()) /
                  (1000 * 60 * 60 * 24);
        ...
        if (pickIds.length > 0) {
          await db
            ...
            .where(
              and(eq(prayers.userId, userId), inArray(prayers.id, pickIds)),
            );
        }

        await db
          .insert(dailyPrayerPicks)
          .values({
            userId,
            dateKey: todayKey,
            prayerIds: pickIds,
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const [updated] = await db
          ...
          .where(eq(prayerStreak.id, streak.id))
          .returning();
        ...
        try {
          ...
          console.error(
            "[prayer-streak] Failed to update lastPrayedAt for daily picks:",
            err,
          );
        }
    ...
    async (_req: Request, res: Response) => {
      try {
        const rows = await db
          ...
          .from(appSettings)
          .where(
            inArray(appSettings.key, ["autoArchiveEnabled", "autoArchiveDays"]),
          );
        let enabled = true;
        let days = 90;
        for (const row of rows) {
          if (row.key === "autoArchiveEnabled") enabled = row.value === "true";
          if (row.key === "autoArchiveDays")
            days = parseInt(row.value, 10) || 90;
        }
  ...
  app.get(
    "/api/officials/:id/prayer-counts",
    requireUser,
    async (req: Request, res: Response) => {
      try {
        const userId = req.userId!;
