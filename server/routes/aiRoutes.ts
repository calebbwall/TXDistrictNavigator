  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        res
          .status(500)
          .json({ error: "Failed to parse search. Please try again." });
      }
    },
  );
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      ...
      if (!process.env.GROQ_API_KEY) {
        return res
          .status(503)
          .json({ error: "AI summarization is not configured" });
      }
      ...
      try {
        ...
        res
          .status(500)
          .json({ error: "Failed to summarize bill. Please try again." });
      }
    },
  );
    ...
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        const partyCode = entities.party
          ? entities.party.toLowerCase().startsWith("r")
            ? "R"
            : entities.party.toLowerCase().startsWith("d")
              ? "D"
              : null
          : null;
        ...
        const sourceCode = entities.chamber
          ? (sourceMap[entities.chamber] ?? null)
          : null;
        ...
        if (intent === "officials" || intent === "committees") {
          ...
          if (sourceCode)
            conditions.push(eq(officialPublic.source, sourceCode as any));

          if (entities.names?.length) {
            const nameConditions = entities.names.map((n) =>
              ilike(officialPublic.fullName, `%${n}%`),
            );
          ...
          if (entities.keywords?.length) {
            const cityConditions = entities.keywords.map((kw) =>
              ilike(officialPublic.searchCities, `%${kw}%`),
            );
          ...
          if (entities.committeeKeywords?.length) {
            ...
            const cmteConditions = entities.committeeKeywords.map((kw) =>
              ilike(committees.name, `%${kw}%`),
            );
            ...
            if (matchingCommittees.length > 0) {
              ...
              if (memberships.length > 0) {
                ...
                const officialConditions: any[] = [
                  inArray(officialPublic.id, memberIds.slice(0, 60)),
                ];
                if (partyCode)
                  officialConditions.push(eq(officialPublic.party, partyCode));
                if (sourceCode)
                  officialConditions.push(
                    eq(officialPublic.source, sourceCode as any),
                  );
                  ...
                  const party =
                    o.party === "R"
                      ...
                      : o.party === "D"
                        ? "Democrat"
                        : (o.party ?? "Unknown");
                  const role = membership?.roleTitle
                ...
                const unlinkedMembers = memberships.filter(
                  (m) => !m.officialId || !linkedIds.has(m.officialId),
                );
                ...
                const cmteNames = matchingCommittees
                  .map((c) => c.name)
                  .join(", ");
                const partyLabel = partyCode
                  ? partyCode === "R"
                    ? "Republican "
                    : "Democrat "
                  : "";
          ...
          if (!dataContext) {
            const officials = await db
              .select({
                ...
                source: officialPublic.source,
                district: officialPublic.district,
                roleTitle: officialPublic.roleTitle,
            ...
            const lines = officials.map((o) => {
              ...
              const cities = o.searchCities
                ? ` [cities: ${o.searchCities}]`
                : "";
          ...
          if (entities.billNumbers?.length) {
            billConditions.push(
              or(
                ...entities.billNumbers.map((bn) =>
                  ilike(bills.billNumber, `%${bn.replace(/\s+/g, "")}%`),
                ),
              )!,
            );
          ...
          const billsData = await db
            .select({
              ...
              legSession: bills.legSession,
              caption: bills.caption,
            })
            .from(bills)
            .where(
              billConditions.length > 0 ? and(...billConditions) : undefined,
            )
          ...
          if (billsData.length > 0) {
            ...
            const actionsData = await db
              .select({
                ...
                actionText: billActions.actionText,
            ...
            const upcomingAgenda = await db
              ...
              .innerJoin(
                legislativeEvents,
                eq(hearingAgendaItems.eventId, legislativeEvents.id),
              )
              .where(
                and(
            ...
            for (const a of upcomingAgenda) {
              if (a.billNumber && !hearingByBill.has(a.billNumber)) {
                const dateStr = a.hearingDate
                  ? a.hearingDate.toLocaleDateString("en-US", {
                      weekday: "short",
                      month: "short",
                      day: "numeric",
          ...
          if (entities.committeeKeywords?.length) {
            ...
            if (matchingCmtes.length > 0) {
              ...
              hearingConditions.push(
                inArray(legislativeEvents.committeeId, committeeFilter),
              );
            }
          }
          ...
          const events = await db
            ...
            .leftJoin(
              committees,
              eq(committees.id, legislativeEvents.committeeId),
            )
            .leftJoin(
              hearingDetails,
          ...
          if (events.length > 0) {
            ...
            for (const item of agendaItems) {
              ...
              existing.push(
                item.billNumber
                  ? `${item.billNumber}: ${item.itemText}`
                  : item.itemText,
              );
              agendaByEvent.set(item.eventId, existing);
              billCountByEvent.set(
                item.eventId,
                (billCountByEvent.get(item.eventId) ?? 0) + 1,
              );
            }
            ...
            const lines = events.map((e) => {
              ...
              const witnessStr = e.witnessCount
                ? `${e.witnessCount} witnesses`
                : "";
          ...
          const [
            ...
            db
              ...
              .where(
                and(
                  eq(officialPublic.source, "TX_HOUSE"),
            ...
            db
              ...
              .where(
                and(
                  eq(officialPublic.source, "TX_SENATE"),
                  eq(officialPublic.active, true),
                ),
              ),
            db
              ...
              .where(
                and(
                  eq(officialPublic.source, "US_HOUSE"),
                  eq(officialPublic.active, true),
                ),
              ),
            db
          ...
          const hearingLines = nextHearings.map((h) => {
            const dateStr = h.startsAt
              ? h.startsAt.toLocaleString("en-US", {
                  weekday: "short",
                  month: "short",
                  day: "numeric",
                  hour: "numeric",
...
${nextHearings.length > 0 ? `\nNext ${nextHearings.length} upcoming hearings:\n${hearingLines.join("\n")}` : "No upcoming hearings."}`;
        ...
        if (process.env.GOOGLE_SEARCH_API_KEY && process.env.GOOGLE_SEARCH_CX) {
          const searchQuery =
            intent === "legislation" && entities.billNumbers?.length
